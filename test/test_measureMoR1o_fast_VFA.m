function test_measureMoR1o_fast_VFA()
%--------------------------------------------------------------------------
% FUNCTION: test_measureMoR1o_fast_VFA
%
% PURPOSE:
%   Runs a series of synthetic tests on the function
%   `measureMoR1o_fast_VFA` to validate its robustness.
%
...
%--------------------------------------------------------------------------
    ...
    T1o_values = [0.5, 1.0, 1.5, 2.0];         % Simulated T1 relaxation times (seconds)

    %-----------------------------
    % MRI Acquisition Parameters
    %-----------------------------
    TR       = 0.015;                          % Fixed repetition time (seconds)
    flip_deg = linspace(2, 25, 10)';          % Flip angles in degrees (vector)

    repetition = 3;                            % Number of random initial guesses per fit
    ...
    fprintf('Running multi-case test for measureMoR1o_fast_VFA...\n\n');
    ...
    for i = 1:length(Mo_values)
        for j = 1:length(T1o_values)
            ...
            %-----------------------------
            [Mo_fit, R1o_fit, sse] = measureMoR1o_fast_VFA( ...
                                      noisy_signal, flip_deg, TR, doPlot, repetition);
...
end % end of test_measureMoR1o_fast_VFA
